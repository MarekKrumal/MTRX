...
const Converstaion = ({ conversation }) => {
    ...
    const lastMessage = conversation.lastMessage || { text: '', sender: null };
        ...
        >
            ...
            <Stack direction={"column"} fontSize={"sm"}>
                <Text fontWeight="700" display={"flex"} alignItems={"center"}>
                    {user.username || 'Unknown User'} <Image src="/verified.png" w={4} h={4} ml={1} />
                </Text>
                <Text fontSize={"xs"} display={"flex"} alignItems={"center"} gap={1}>
                    {currentUser._id === lastMessage.sender ? <BsCheck2All size={16} /> : ""}
                    {lastMessage.text.length > 18 ? lastMessage.text.substring(0, 50) + "..." : lastMessage.text}
                </Text>
            </Stack>
        </Flex>
    );
